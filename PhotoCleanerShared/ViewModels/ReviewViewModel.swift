...
final class ReviewViewModel: ObservableObject {
    private enum PersistenceKeys {
        static let progressPrefix = "review.progress"
    }
    ...
    @Published var shouldShowJumpBrowser = false
    ...
    private var preloadedImages: [String: PlatformImage] = [:]

#if os(iOS)
    private let displayTargetSize = CGSize(width: 1600, height: 1600)
#else
    ...
    var currentProgressKey: String {
        "\(PersistenceKeys.progressPrefix).\(filter.rawValue).\(sortOrder.rawValue)"
    }
    ...
    func jumpToAsset(localIdentifier: String) {
        guard let index = assets.firstIndex(where: { $0.localIdentifier == localIdentifier }) else { return }
        jumpToAsset(at: index)
    }

    func jumpToAsset(at index: Int) {
    ...
    func requestThumbnail(for asset: PHAsset, targetSize: CGSize = CGSize(width: 220, height: 220)) async -> PlatformImage? {
        do {
            return try await photosService.requestImage(for: asset, targetSize: targetSize)
        } catch {
            return nil
        }
    }
    ...
    private func startPreloadingNearbyAssets() {
        preloadTask?.cancel()

        var neighboringAssets: [PHAsset] = []
        let previousIndex = currentIndex - 1
        let nextIndex = currentIndex + 1

        if assets.indices.contains(previousIndex) {
            neighboringAssets.append(assets[previousIndex])
        }
        if assets.indices.contains(nextIndex) {
            neighboringAssets.append(assets[nextIndex])
        }

        guard neighboringAssets.isEmpty == false else { return }

        preloadTask = Task { [weak self] in
            guard let self else { return }

            for asset in neighboringAssets {
                if Task.isCancelled {
                    return
                }
    ...
    private func saveCurrentProgress() {
        ...
        UserDefaults.standard.set(identifier, forKey: currentProgressKey)
    }

    private func restoreProgressIfAvailable() {
        guard let savedIdentifier = UserDefaults.standard.string(forKey: currentProgressKey) else { return }
        guard let savedIndex = assets.firstIndex(where: { $0.localIdentifier == savedIdentifier }) else { return }
